...
(function($) {
	...
	// needs_SOAPAction should be true if the operation does a write, false if only a read
	var WSops = new Array();
	...
	$.fn.SPServices = function(options) {
		...
		switch(opt.operation) {
			...
			case "AddComment":
				SOAPEnvelope.payload += wrapNode("url", opt.url);
				SOAPEnvelope.payload += wrapNode("comment", opt.comment);
				SOAPEnvelope.payload += wrapNode("isHighPriority", opt.isHighPriority);
				SOAPEnvelope.payload += wrapNode("title", opt.title);
				break;
			case "AddTag":
				SOAPEnvelope.payload += wrapNode("url", opt.url);
				SOAPEnvelope.payload += wrapNode("termID", opt.termID);
				SOAPEnvelope.payload += wrapNode("title", opt.title);
				SOAPEnvelope.payload += wrapNode("isPrivate", opt.isPrivate);
				break;
			case "AddTagByKeyword":
				SOAPEnvelope.payload += wrapNode("url", opt.url);
				SOAPEnvelope.payload += wrapNode("keyword", opt.keyword);
				SOAPEnvelope.payload += wrapNode("title", opt.title);
				SOAPEnvelope.payload += wrapNode("isPrivate", opt.isPrivate);
				break;
			case "CountCommentsOfUser":
				SOAPEnvelope.payload += wrapNode("userAccountName", opt.userAccountName);
				break;
			...
			case "DeleteComment":
				SOAPEnvelope.payload += wrapNode("url", opt.url);
				SOAPEnvelope.payload += wrapNode("lastModifiedTime", opt.lastModifiedTime);
				break;
			case "DeleteRating":
				SOAPEnvelope.payload += wrapNode("url", opt.url);
				break;
			...
			case "DeleteTags":
				...
				break;
			case "GetAllTagTerms":
				SOAPEnvelope.payload += wrapNode("maximumItemsToReturn", opt.maximumItemsToReturn);
				break;
			case "GetAllTagTermsForUrlFolder":
				SOAPEnvelope.payload += wrapNode("urlFolder", opt.urlFolder);
				SOAPEnvelope.payload += wrapNode("maximumItemsToReturn", opt.maximumItemsToReturn);
				break;
			case "GetAllTagUrls":
				SOAPEnvelope.payload += wrapNode("termID", opt.termID);
			...
			case "GetCommentsOfUser":
				SOAPEnvelope.payload += wrapNode("userAccountName", opt.userAccountName);
				SOAPEnvelope.payload += wrapNode("maximumItemsToReturn", opt.maximumItemsToReturn);
				SOAPEnvelope.payload += wrapNode("startIndex", opt.startIndex);
				break;
			...
			case "SetRating":
				SOAPEnvelope.payload += wrapNode("url", opt.url);
				SOAPEnvelope.payload += wrapNode("rating", opt.rating);
				SOAPEnvelope.payload += wrapNode("title", opt.title);
				SOAPEnvelope.payload += wrapNode("analysisDataEntry", opt.analysisDataEntry);
				break;
			case "UpdateComment":
				SOAPEnvelope.payload += wrapNode("url", opt.url);
				SOAPEnvelope.payload += wrapNode("lastModifiedTime", opt.lastModifiedTime);
			...
			case "AddUserToGroup":
				...
				SOAPEnvelope.payload += wrapNode("userEmail", opt.userEmail);
				SOAPEnvelope.payload += wrapNode("userNotes", opt.userNotes);
				break;
			case "AddUserToRole":
				SOAPEnvelope.payload += wrapNode("roleName", opt.roleName);
				SOAPEnvelope.payload += wrapNode("userName", opt.userName);
			...
			case "CreateContentType":
				SOAPEnvelope.payload += wrapNode("displayName", opt.displayName);
				SOAPEnvelope.payload += wrapNode("parentType", opt.parentType);
				SOAPEnvelope.payload += wrapNode("newFields", opt.newFields);
				SOAPEnvelope.payload += wrapNode("contentTypeProperties", opt.contentTypeProperties);
				break;
			case "GetColumns":
			...
			case "GetCustomizedPageStatus":
				SOAPEnvelope.payload += wrapNode("fileUrl", opt.fileUrl);
				break;
	...
	$.fn.SPServices.defaults = {
		...
		contentTypeProperties: "",	// A string that represents the properties to specify for the new site content type.
	...
	$.fn.SPServices.SPListNameFromUrl = function () {
		...
		var listPath = unescapeUrl(thisPageBaseName.substring(0, thisPageBaseName.lastIndexOf(SLASH) + 1)).toUpperCase();
		...
		// Return the GUID (ID)
		return thisList;
	};// End of function SPListNameFromUrl
	...
	$.fn.SPServices.SPUpdateMultipleListItems = function (options) {
		...
		for(var i=0; i < itemsToUpdate.length; i++) {
			...
			for (var fieldNum=0; fieldNum < opt.valuepairs.length; fieldNum++) {
