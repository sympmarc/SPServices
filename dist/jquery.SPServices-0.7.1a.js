	...
	function choiceMultiToJsonObject(s) {
		if(s.length === 0) {
			...
			for(i=0; i < thisChoiceMulti.length; i++) {
				if(thisChoiceMulti[i].length !== 0) thisChoiceMultiObject.push(thisChoiceMulti[i]);
			}
			...
			case "LookupMulti":
				...
				if(columnValue.length > 0) {
					...
					for (i=0; i < lookupMultiValues.length / 2; i++) {
						outString += "<a href='" + webUrl + SLASH + dispUrl +
	...
	function getDropdownSelected(columnSelect, matchOnId) {
		...
		switch(columnSelect.Type) {
			...
			case "C":
				...
				break;
			case "M":
				var columnSelections = columnSelect.Obj.closest("td").next(":eq(4)").find("select[ID$='SelectResult']");
