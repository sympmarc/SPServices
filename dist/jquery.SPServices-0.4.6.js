...
(function($) {
		...
		switch(parentSelect.Type) {
			...
			case "M":
				if(opt.debug) errBox("SPServices.SPCascadeDropdowns",
					"parentColumn: " + opt.parentColumn,
					"Multi-select columns not yet supported by this function");
				break;
			default:
				break;
		}
	...
	function cascadeDropdown(opt) {
		...
		if(parentSelect.Obj.attr("cascadeDropdownSelected_" + opt.childColumn) == parentSelectSelected) return;
		parentSelect.Obj.attr("cascadeDropdownSelected_" + opt.childColumn, parentSelectSelected);
			...
			completefunc: function(xData, Status) {
				...
				$(xData.responseXML).find("[nodeName=z:row]").each(function() {
					...
					switch(childSelect.Type) {
						case "S":
							...
							childSelect.Obj.append("<option" + selected + " value='" + $(this).attr("ows_ID") + "'>" + thisOptionValue + "</option>");
							break;
						case "C":
							if (thisOptionValue == childSelectSelected) childSelect.Obj.attr("value", childSelectSelected);
							choices = choices + "|" + thisOptionValue + "|" + $(this).attr("ows_ID");
							break;
						case "M":
							childSelect.Obj.append("<option value='" + $(this).attr("ows_ID") + "'>" + $(this).attr("ows_" + opt.relationshipListChildColumn) + "</option>");
							newMultiLookupPickerdata += $(this).attr("ows_ID") + "|t" + $(this).attr("ows_" + opt.relationshipListChildColumn) + "|t |t |t";
							break;
						default:
							break;
					}
				...
				switch(childSelect.Type) {
					...
					case "M":
						...
						// Hide any options ni the candidate list which are already selected
						$(childSelect.Obj).find("option").each(function() {
							var thisSelected = $(this);
							$(currentSelection).find("option").each(function() {
	...
	function showRelated(opt) {
		...
		if(opt.CAMLQuery.length > 0) camlQuery += "<And>";
		camlQuery += "<Eq><FieldRef Name='" + opt.relatedListColumn + "'/><Value Type='Text'>" + columnSelectSelected + "</Value></Eq>";
		if(opt.CAMLQuery.length > 0) camlQuery += opt.CAMLQuery + "</And>";
		camlQuery += "</Where></Query>";
