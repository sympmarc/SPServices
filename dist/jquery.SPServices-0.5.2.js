...
(function($) {
	...
	// Array to store Web Service / operation associations
	var WSops = new Array();
	...
	WSops["GetListContentType"]						= LISTS;
	WSops["GetListContentTypes"]					= LISTS;
	WSops["GetListItems"]							= LISTS;
	WSops["UpdateList"]								= LISTS;
	WSops["UpdateListItems"]						= LISTS;

	WSops["AddMeeting"]								= MEETINGS;
	WSops["CreateWorkspace"]						= MEETINGS;
	WSops["RemoveMeeting"]							= MEETINGS;
	WSops["SetWorkSpaceTitle"]						= MEETINGS;

	WSops["SearchPrincipals"]						= PEOPLE;

	WSops["AddPermission"]							= PERMISSIONS;
	WSops["AddPermissionCollection"]				= PERMISSIONS;
	WSops["GetPermissionCollection"]				= PERMISSIONS;
	WSops["RemovePermission"]						= PERMISSIONS;
	WSops["RemovePermissionCollection"]				= PERMISSIONS;
	...
	WSops["ModifyUserPropertyByAccountName"]		= USERPROFILESERVICE;
	...
	WSops["AddWebPart"]								= WEBPARTPAGES;
	WSops["GetWebPart2"]							= WEBPARTPAGES;
	WSops["GetWebPartPage"]							= WEBPARTPAGES;
	WSops["GetWebPartProperties"]					= WEBPARTPAGES;
	...
	$.fn.SPServices.SPArrangeChoices = function (options) {
		...
		$("td.ms-formbody").each(function() {
			// Cherck for the right comment
			if(searchText.test($(this).html())) {
				// Put all of the choices into the options array
				...
				// Add all of the option to the out string
	...
	$.fn.SPServices.SPGetQueryString = function() {
		...
		for (var i=0; i < args.length; i++) {
			...
			nameVal[1] = temp.join(' ');
			queryStringVals[nameVal[0]] = nameVal[1];
		}
	...
	function listNameFromUrl() {
		...
		var listPath = unescapeUrl(thisPageBaseName.substring(0, thisPageBaseName.lastIndexOf(SLASH)));
		...
		$().SPServices({
			...
			completefunc: function(xData, Status) {
				$(xData.responseXML).find("List").each(function() {
					...
					var listCollList = defaultViewUrl.substring(0, defaultViewUrl.lastIndexOf(SLASH));
