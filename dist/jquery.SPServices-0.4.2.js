...
(function($) {
	...
	WSops["CopyIntoItemsLocal"]						= "Copy";

	WSops["GetForm"]								= "Forms";
	WSops["GetFormCollection"]						= "Forms";
	
	WSops["AddList"]								= "Lists";
	WSops["CheckInFile"]							= "Lists";
	WSops["CheckOutFile"]							= "Lists";
	...
	$.fn.SPServices.SPLookupAddNew = function(options) {	
		...
		if(newUrl.length > 0) {
			...
			newLink = "<a href='" + lookupListUrl + "/" + newUrl + "?Source=" + location.href + "'>" + opt.promptText.replace(/\{0\}/g, opt.lookupColumn) + "</a>";
			// Append the link to the Lookup columns's formbody table cell
			$("select[title=" + opt.lookupColumn + "]").parents("td.ms-formbody").append(newLink);
		}
	};
	...
	$.fn.SPServices.SPGetDisplayFromStatic = function (options) {
		...
		$().SPServices({
			...
			completefunc: function(xData, Status) {
				$(xData.responseXML).find("Field").each(function() {
					//alert(opt.columnDisplayName + $(this).attr("Name") + $(this).attr("StaticName"));
	...
	$.fn.SPServices.SPRedirectWithID = function (options) {
		...
		if(vals["ID"] == undefined) {
			...
			$().find("form[name='aspnetForm']").each(function() {
				// Add the last ID this user has created to the postback action
				var thisSource = "Source=" + vals["Source"].replace(/\//g, "%2f").replace(/:/g, "%3a");
				var thisUrl = location.href.substring(0, location.href.indexOf("?"));
				var newAction = thisUrl + "?Source=" + thisUrl + "?ID=" + lastID +
					"%26RealSource=" + vals["Source"] +
					((vals["RedirectURL"] != undefined) ? ("%26RedirectURL=" + vals["RedirectURL"]) : "");
				$(this).attr("action", newAction);
			});
		// If this is the load after the item is saved, wait until the new item has been saved, then do the redirect
		// to redirectUrl with the new lastID, passing along the original Source.
		} else {
			...
			//alert("RedirectURL=" + vals["RedirectURL"] + " opt.redirectUrl=" + opt.redirectUrl);
			//alert(vals["RedirectURL"] == undefined);
			var thisRedirectUrl = (vals["RedirectURL"] == undefined) ? opt.redirectUrl : vals["RedirectURL"];
			location.href = thisRedirectUrl + "?ID=" + lastID + "&Source=" + vals["RealSource"];
		}
	...
	function listNameFromUrl() {
		...
		var thisList = listPath.substr(0,listPath.search("/"));
		return thisList;
	}
