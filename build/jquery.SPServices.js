...
*/
;(function() {
var src_utils_constants, src_utils_SPServicesutils, src_utils_getDropdownSelected, src_core_SPServicescore, src_SPServices;
(function (factory) {
  ...
  src_utils_getDropdownSelected = function ($, constants) {
    /**
  ...
  src_core_SPServicescore = function ($, utils, constants, getDropdownSelected) {
    // Version info
    var VERSION = '2014.02';
    // TODO: Update version
    // String constants
    ...
    // Global variables
    var currentContext = new SPServicesContext();
    ...
    $.fn.SPServices.SPGetCurrentSite = function () {
      ...
      var msg = SOAPEnvelope.header + '<WebUrlFromPageUrl xmlns=\'' + SCHEMASharePoint + '/soap/\' ><pageUrl>' + (location.href.indexOf('?') > 0 ? location.href.substr(0, location.href.indexOf('?')) : location.href) + '</pageUrl></WebUrlFromPageUrl>' + SOAPEnvelope.footer;
      $.ajax({
        async: false,
    ...
    $.fn.SPServices.SPRedirectWithID = function (options) {
      ...
      if (typeof queryStringVals.ID === 'undefined') {
        lastID = $().SPServices.SPGetLastItemId({ listName: currentContext.thisList });
                          ...
                          if ($(this).attr('ID').substring(0, 6) !== '0x0120') {
                            ...
                            for (i = 0; i < formTypes.length; i++) {
                              ...
                              if (!formTypes[i][2]) {
                                ...
                                SPScriptAuditPage(opt, listXml, 'Form', formTypes[i][0], defaultViewUrl.substring(0, defaultViewUrl.lastIndexOf(SLASH) + 1) + formTypes[i][1]);
                              }
                            }
    ...
    $.fn.SPServices.SPListNameFromUrl = function (options) {
      ...
      if (opt.listName.length > 0) {
        ...
        return currentContext.thisList;  // Do we already know the current list?
    ...
    $.fn.SPServices.SPDropdownCtl = function (options) {
      ...
      if (columnObj.Type === constants.dropdownType.complex) {
        ...
        columnObj.master = window[addButtonId.replace(/AddButton/, multiLookupPrefix + '_m')] || // SharePoint 2007
    ...
    function showColumn(listXML, columnXML, columnValue, opt) {
      ...
      case 'Lookup':
        ...
        case 'FileRef':
          ...
          dispUrl = listXML.attr('BaseType') === '1' ? listXML.attr('RootFolder') + SLASH + 'Forms/DispForm.aspx' : listXML.attr('RootFolder') + SLASH + 'DispForm.aspx';
        ...
        default:
          ...
          outString = '<a href=\'' + opt.relatedWebURL + SLASH + dispUrl + '?ID=' + columnValue.substring(0, columnValue.search(constants.spDelim)) + '&RootFolder=*&Source=' + utils.escapeUrl(location.href) + '\'>' + columnValue.substring(columnValue.search(constants.spDelim) + 2) + '</a>';
          break;
        }
        break;
      case 'LookupMulti':
        ...
        if (columnValue.length > 0) {
          ...
          for (i = 0; i < lookupMultiValues.length / 2; i++) {
            outArray.push('<a href=\'' + webUrl + SLASH + dispUrl + '?ID=' + lookupMultiValues[i * 2] + '&RootFolder=*&Source=' + utils.escapeUrl(location.href) + '\'>' + lookupMultiValues[i * 2 + 1] + '</a>');
          }
        }
        outString = outArray.join(', ');
        break;
  ...
  }(jquery, src_utils_SPServicesutils, src_utils_constants, src_utils_getDropdownSelected);
